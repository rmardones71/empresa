...
function assertField(resource, fieldName) {
  const allowed = new Set([resource.idField, ...fieldNames(resource), ...(resource.joins || []).map((join) => join.as)])
...
function buildSelect(resource) {
  const baseColumns = [resource.idField, ...fieldNames(resource)]
  ...
  return {
    selectSql: [...selectColumns, ...joinColumns].join(',\n      '),
    joins,
  }
}
...
async function list(resource, { page, pageSize, q, sortBy, sortDir }) {
  ...
  const sortPrefix = (resource.joins || []).some((join) => join.as === sortField) ? '' : 'base.'
...
async function update(resource, id, data) {
  const updateFields = fieldNames(resource, { includeCreateOnly: false }).filter((field) => {
    ...
    return config?.includeOnUpdate !== false && data[field] !== undefined
  })

  const setSql = updateFields.map((field) => `${field} = @${field}`).join(', ')
  await query(
    `
...
module.exports = { parseId, list, getById, create, update, remove, countDependency, listLookup }
