...
function getPool() {
  if (!poolPromise) {
    ...
    const config = env.db.integrated
      ? {
          ...common,
          connectionString: `Driver={${env.db.odbcDriver}};Server=${integratedServer};Database=${env.db.database};Trusted_Connection=Yes;Encrypt=${env.db.encrypt ? 'Mandatory' : 'Optional'};TrustServerCertificate=Yes;`,
        }
      : {
          ...common,
          user: env.db.user,
